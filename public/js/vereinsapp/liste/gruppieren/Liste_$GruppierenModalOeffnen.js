...
function Liste_$GruppierenModalOeffnen($werkzeug) {
    ...
    const $modal = Dom_$ModalInitialisiertZurueck(
        Liste_ElementBeschriftungErsetztZurueck(Util_WertBereinigtZurueck($werkzeug.attr("modal_title"), undefined), {
            liste1: liste,
        }),
    ...
    const $gruppieren_eigenschaft = $modal.find(".gruppieren_eigenschaft");
    $gruppieren_eigenschaft.attr("liste", liste);
    const $gruppieren_wert = $modal.find(".gruppieren_wert");
    ...
    Dom_$Quelle$ZielVerknuepfen($modal.find(".gruppieren_vorgegeben, .gruppieren_eigenschaft"), $werkzeug);
