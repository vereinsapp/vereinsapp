...
function Liste_Element$NavigationAktualisieren($element_navigation, $element) {
    ...
    // TABELLE FILTERN
    const filtern_data = Liste_WertBereinigtZurueck($element_navigation.attr("filtern"), new Object());
    ...
    // TABELLE SORTIEREN
    const sortieren_data = Liste_WertBereinigtZurueck($element_navigation.attr("sortieren"), undefined);
    ...
    $.each(tabelle_gefiltert_sortiert, function (position, element) {
        if (element.id === Liste_WertBereinigtZurueck($element.attr(LISTEN[liste].element + "_id"), undefined)) {
