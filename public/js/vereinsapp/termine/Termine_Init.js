...
ELEMENTE.rueckmeldung.zuordnen_aktion = function (rueckmeldung) {
    const rueckmeldung_id = Number(rueckmeldung.id);
    const status = Number(rueckmeldung_id);

    if ("termine" in LISTEN) {
        const termin = LISTEN.termine.tabelle[Number(rueckmeldung.termin_id)];

        if (typeof termin !== "undefined" && "zugeordnete_elemente_nach_liste" in termin) {
            const zugeordnete_elemente_nach_liste = termin.zugeordnete_elemente_nach_liste;

            if (!("rueckmeldungen" in zugeordnete_elemente_nach_liste)) zugeordnete_elemente_nach_liste.rueckmeldungen = new Object();
            zugeordnete_elemente_nach_liste.rueckmeldungen[status] = rueckmeldung_id;
        }
    }

    if ("mitglieder" in LISTEN) {
        const mitglied = LISTEN.mitglieder.tabelle[Number(rueckmeldung.mitglied_id)];
...
ELEMENTE.anwesenheit.zuordnen_aktion = function (anwesenheit) {
    const anwesenheit_id = Number(anwesenheit.id);
    const status = Number(anwesenheit_id);
    ...
    if ("termine" in LISTEN) {
        ...
        if (typeof termin !== "undefined" && "zugeordnete_elemente_nach_liste" in termin) {
            ...
            if (!("anwesenheiten" in zugeordnete_elemente_nach_liste)) zugeordnete_elemente_nach_liste.anwesenheiten = new Object();
            zugeordnete_elemente_nach_liste.anwesenheiten[status] = anwesenheit_id;
        }
    }

    if ("mitglieder" in LISTEN) {
        const mitglied = LISTEN.mitglieder.tabelle[Number(anwesenheit.mitglied_id)];
