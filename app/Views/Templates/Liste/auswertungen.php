<div id="<?= $auswertungen['instanz']; ?>" class="auswertungen w-100" auswertungen="<?= $auswertungen['auswertungen']; ?>" liste="<?= $auswertungen['liste']; ?>"<?php
    ...
    if( array_key_exists( 'werkzeuge', $auswertungen ) AND is_array( $auswertungen['werkzeuge'] ) AND count( $auswertungen['werkzeuge'] ) > 0 ) { ?> werkzeuge='<?= json_encode( $auswertungen['werkzeuge'], JSON_UNESCAPED_UNICODE ); ?>'<?php }
    ?>>
    <div class="meta w-100 text-center invisible"><span class="werkzeuge float-end"></span></div>
