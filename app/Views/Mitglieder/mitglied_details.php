...
<div class="row row-cols-1 row-cols-lg-2 gy-3 gx-0 gx-lg-3 w-100">
    ...
    <div class="col"><div class="card element" liste="mitglieder" mitglied_id="<?= $liste['alle_mitglieder']['mitglied_id']; ?>"<?php
        if( array_key_exists( 'werkzeuge', $liste['alle_mitglieder']['element'] ) AND is_array( $liste['alle_mitglieder']['element']['werkzeuge'] ) AND count( $liste['alle_mitglieder']['element']['werkzeuge'] ) > 0 ) { ?> werkzeuge='<?= json_encode( $liste['alle_mitglieder']['element']['werkzeuge'], JSON_UNESCAPED_UNICODE ); ?>'<?php }
    ?>>
